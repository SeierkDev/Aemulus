...
function rowToRun(row: Record<string, unknown>, steps: RunStepRecord[]): Run {
  return {
    ...
    skillVersion: row.skill_version == null ? null : Number(row.skill_version),
    agencHash: row.agenc_hash == null ? null : String(row.agenc_hash),
    agencCommitment: row.agenc_commitment == null ? null : String(row.agenc_commitment),
