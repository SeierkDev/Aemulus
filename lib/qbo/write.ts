...
// Idempotent invoice → QuickBooks Bill write.
//
// The (invoice_id) primary key on qbo_write is the double-post guard: only the
// caller that successfully claims the row writes to QBO. Concurrent duplicates
...
export async function readQboWrite(invoiceId: string): Promise<QboWriteRow | null> {
  await ensureQboSchema();
  const r = await db.execute({ sql: `SELECT * FROM qbo_write WHERE invoice_id = ?`, args: [invoiceId] });
  const row = r.rows[0];
  if (!row) return null;
  return {
...
async function setPosted(invoiceId: string, billId: string, now: number) {
  await db.execute({
    sql: `UPDATE qbo_write SET status='posted', qbo_bill_id=?, last_error=NULL, updated_at=? WHERE invoice_id=?`,
    args: [billId, now, invoiceId],
  });
}
async function setFailed(invoiceId: string, error: string, now: number) {
  await db.execute({
    sql: `UPDATE qbo_write SET status='failed', last_error=?, updated_at=? WHERE invoice_id=?`,
    args: [error.slice(0, 500), now, invoiceId],
  });
}
...
export async function writeInvoiceToQbo(input: WriteInvoiceInput): Promise<WriteResult> {
  ...
  const claim = await db.execute({
    sql: `INSERT OR IGNORE INTO qbo_write (invoice_id, status, doc_number, attempts, created_at, updated_at)
          VALUES (?, 'pending', ?, 1, ?, ?)`,
    args: [invoiceId, docNumber, now, now],
  });
  let owned = claim.rowsAffected === 1;

  if (!owned) {
    const row = await readQboWrite(invoiceId);
    ...
    if (existing?.Id) {
      await setPosted(invoiceId, existing.Id, now);
      return { status: "posted", billId: existing.Id };
    }
    if (row.status === "failed") {
      const r = await db.execute({
        sql: `UPDATE qbo_write SET status='pending', attempts=attempts+1, last_error=NULL, updated_at=? WHERE invoice_id=? AND status='failed'`,
        args: [now, invoiceId],
      });
      owned = r.rowsAffected === 1;
    } else {
      // pending — reclaim only if the claim is stale (crashed), else back off.
      if (now - (await staleUpdatedAt(invoiceId)) > STALE_MS) {
        const r = await db.execute({
          sql: `UPDATE qbo_write SET attempts=attempts+1, updated_at=? WHERE invoice_id=? AND status='pending' AND updated_at < ?`,
          args: [now, invoiceId, now - STALE_MS],
        });
        owned = r.rowsAffected === 1;
      }
    }
  ...
  try {
    ...
    await setPosted(invoiceId, bill.Id, now);
    ...
    await setFailed(invoiceId, msg, now);
    return { status: "failed", error: msg };
  }
}

async function staleUpdatedAt(invoiceId: string): Promise<number> {
  const r = await db.execute({ sql: `SELECT updated_at FROM qbo_write WHERE invoice_id = ?`, args: [invoiceId] });
